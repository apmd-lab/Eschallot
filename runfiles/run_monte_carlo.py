...
fine_roughness = 0
coarse_roughness = 0
layer_thickness = np.array([200000]) # thickness of each layer (nm)
...
f_vol_tot = np.array([0.4]) # total particle volume fraction for each layer
f_num = np.array([[0.5,0.5]]) # relative number densities of different particle types (should add up to 1; dimension -> n_layer x n_type; n_type=1 for unimodal, 2 for bimodal, etc.)
r_mean = np.array([[250,500]]) # mean radii (dimension -> n_layer x n_type)
scale = np.array([[1,0.1]]) # determines the spread of the gamma distribution (smaller scale -> larger spread)
...
for n_t in range(n_type):
    for p in range(n_particle):
        if n_t == 0:
            config[n_t,p] = np.array(['ZrO2_li']) # set the material of particle type 1
        elif n_t == 1:
            config[n_t,p] = np.array(['BaSO4_li']) # set the material of particle type 2
...
identifier = 'bimodal_distribution'
